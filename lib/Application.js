...
let AboutMe = {
  ...
  _latestBookmark: "",
  factory: {
    contract: "@mozilla.org/network/protocol/about;1?what=me",
    ...
    Component: Class({
      ...
      newChannel: function(uri) {
        let chan = Services.io.newChannel(data.url("about-me.html"), null, null);
  ...
  observe : function(aSubject, aTopic, aData) {
    ...
    switch (aTopic) {
      case "chart-update":
        for (let worker of AboutMe._workers) {
  ...
  page: {
    ...
    onAttach: function(worker) {
      Services.obs.addObserver(AboutMe, "chart-update", false);
      ...
      AboutMe._workers.push(worker); // Set worker so that callback functions can access it after a page refresh.
      ...
      worker.port.on("uninstall_addon", () => {
        StudyApp.uninstall();
      });
...
let StudyApp = {
  ...
  init: function _init(controller) {
    ...
    // about interests
    Factory(AboutMe.factory);
    PageMod(AboutMe.page);
  ...
  start: function _start({loadReason}) {
    ...
    tabs.on('activate', function () {
      if (tabs.activeTab.url == "about:me") {
