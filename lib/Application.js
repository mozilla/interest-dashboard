  ...
  page: {
    ...
    onAttach: function(worker) {
      ...
      worker.port.on("category_visit_request", (data) => {
        if (storage.chartData.interestDashboardData.historyVisits[data.categoryName].complete) {
          ...
          .getNextHistoryVisitPage(data.categoryName).then(() => {
            worker.port.emit("message", {content: {topic: "append_visit_data",
                            data: {"category": data.categoryName,
                                   ...
                                   "pageNum": storage.chartData.interestDashboardData.historyVisits[data.categoryName].pageNum,
      ...
      worker.port.on("chart_data_request", () => {
        if (storage.chartData) {
          ...
          let faviconPromise = StudyApp.controller._streamObjects
            .interestDashboardDataProcessorBolt.getFaviconsForTopSites();
          let visitDetailsPromise = StudyApp.controller._streamObjects
            .interestDashboardDataProcessorBolt.populateHistoryVisitDetails();

          Promise.all([faviconPromise, visitDetailsPromise]).then(() => {
