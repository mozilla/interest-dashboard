...
let AboutInterests = {
  ...
  processingDaysLeft: function(dailyInterestsSpout) {
    for (let worker of AboutInterests._workers) {
      worker.port.emit("message", {content: {topic: "days_left", data: dailyInterestsSpout.numFromToday}});
    }
  },
  ...
  page: {
    ...
    onAttach: function(worker) {
      Services.obs.addObserver(AboutInterests, "chart-update", false);

      StudyApp.controller._streamObjects.dailyInterestsSpout.setEmitCallback(dailyInterestsSpout => {
        worker.port.emit("message", {content: {topic: "days_left", data: dailyInterestsSpout.numFromToday}});
      });

      AboutInterests._workers.push(worker); // Set worker so that callback functions can access it after a page refresh.
...
let AboutKeywords = {
  ...
  historySubmitComplete: function() {
    for (let worker of AboutKeywords._workers) {
      worker.port.emit("message", {content: {topic: "topKeywords",
        data: { keywords: StudyApp.controller.getTopKeywords(50), submitComplete: true}}});
    }
  },
