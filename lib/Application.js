...
let AboutInterests = {
  factory: {
    contract: "@mozilla.org/network/protocol/about;1?what=interests",
    ...
    Component: Class({
      ...
      newChannel: function(uri) {
        let chan = Services.io.newChannel(data.url("about-interests.html"), null, null);
        chan.originalURI = uri;
        return chan;
      },
  ...
  processingDaysLeft: function(daysLeft, jsonData) {
    AboutInterests.worker.port.emit("message", {content: {topic: "days_left", data: daysLeft}});
    AboutInterests.worker.port.emit("message", {content: {topic: "json_update", data: jsonData}});
  },

  historySubmitComplete: function() {
    AboutInterests.worker.port.emit("message", {content: {topic: "ranking_data",
  ...
  page: {
    ...
    contentScriptFile: [
      ...
      data.url("DataProcessor.js"),
      data.url("about-interests.js"),
    ],

    include: ["about:interests"],
    onAttach: function(worker) {
      AboutInterests.worker = worker; // Set worker so that callback functions can access it after a page refresh.
      worker.port.emit("style", data.url("css/devmenu/bootstrap.min.css"));
      worker.port.emit("style", data.url("css/devmenu/bootstrap-responsive.min.css"));
      worker.port.emit("style", data.url("css/devmenu/styles.css"));
      worker.port.emit("style", data.url("vendor/nvd3/nv.d3.min.css"));

      worker.port.emit("message", {content: {topic: "ranking_data", data: { rankings: StudyApp.controller.getRankedInterests(), submitComplete: false}}});
      worker.port.emit("message", {content: {topic: "chart_init", data: storage.chartData}});

      worker.port.on("json_store", function(data) { storage.chartData = data; })
      worker.port.on("history_process", () => {
        storage.chartData = {};
        StudyApp.controller.resubmitHistory({report: AboutInterests.processingDaysLeft, flush: true}).then(AboutInterests.historySubmitComplete);
      });
    }
  },
};
