...
let StudyApp = {
  ...
  addonEventListener: {
    onUninstalling: function(addon) {
      if (addon.id == id) {
        Task.spawn(function() {
          ...
          yield StudyApp.controller.onUninstalling();
          StudyApp.stopCountdown();
    ...
    onDisabling: function(addon){
      if (addon.id == id) {
        Task.spawn(function() {
          yield StudyApp.closeAppTabs();
          yield StudyApp.controller.onDisabling();
          StudyApp.stopCountdown();
          PrefsManager.unsetObservers();
        });
      }
    ...
    onEnabled: function(addon) {
      if (addon.id == id) {
        Task.spawn(function() {
          yield StudyApp.controller.onEnabled();
        });
      }
    },
  },
  ...
  init: function _init(controller) {
    ...
    AddonManager.getAddonByID(id, addon => {
      if (addon && addon.sourceURI) {
        StudyApp.controller.setSourceUri(addon.sourceURI);
      }
    });
  },
  ...
  start: function _start({loadReason}) {
    if (loadReason == "install") {
      ...
      StudyApp.controller.resubmitHistory().then(() => {
        StudyApp.openConsentTab({inBackground: true});
      });
    }
