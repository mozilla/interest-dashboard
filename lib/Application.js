...
let AboutYou = {
  ...
  page: {
    ...
    onAttach: function(worker) {
      ...
      Dashboard.controller._streamObjects.dailyInterestsSpout.setEmitCallback(dailyInterestsSpout => {
      ...
      worker.port.on("category_visit_request", (data) => {
        ...
        Dashboard.controller._streamObjects.interestDashboardDataProcessorBolt
      ...
      worker.port.on("category_topsites_request", (data) => {
        Dashboard.controller._streamObjects.interestDashboardDataProcessorBolt.getFaviconsForTopSites(data.categoryName).then(() => {
      ...
      worker.port.on("debug_report_request", (data) => {
        let debugLogs = Dashboard.controller._streamObjects.interestDashboardDataProcessorBolt.getDebugLogs();
      ...
      worker.port.on("chart_data_request", () => {
        if (storage.chartData) {
          ...
          Dashboard.controller._streamObjects.interestDashboardDataProcessorBolt.getFaviconsForTopSites().then(() => {
      ...
      worker.port.on("history_process", () => {
        ...
        Dashboard.controller.resubmitHistory({flush: true});
      });
    }
  },
};
...
let Dashboard = {
  ...
  init: function _init(controller) {
    ...
    AddonManager.getAddonByID(id, addon => {
      Dashboard.setSourceUri(addon.sourceURI);
    });
  ...
  uninstall: function _uninstall() {
    console.debug("Dashboard.uninstall");
    AddonManager.getAddonByID(id, addon => {
      Dashboard.closeAppTabs()
        .then(_ => {
          addon.uninstall();
        });
    });
  ...
  start: function _start({loadReason}) {
    console.debug("Dashboard.start: on " + loadReason);
    tabs.on('activate', function () {
      if (tabs.activeTab.url == "about:you") {
        Dashboard.submitPromise = Dashboard.controller.submitHistory();
      }
    });
    Dashboard.saveAddonInfo();
    ...
    if (loadReason == "startup") {
      ...
      Dashboard.submitPromise = Dashboard.controller.resubmitHistory();
    }
  },
