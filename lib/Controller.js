...
Controller.prototype = {
  ...
  submitHistory: function(daysAgo, options={}) {
    let {report,flush} = options;
    ...
    return Task.spawn(function() {
      ...
      let bucket = yield historyReader.resubmitHistory({startDay: startDay});
      storage.lastVisitId = historyReader.getLastVisitId();
      // give the bucket to the DayBuffer
      this._dayBuffer.consume(bucket);
      if (flush) {
  ...
  resubmitFullHistory: function(options={}) {
    storage.lastVisitId = 0;
    this._ranker.clear();
    return this.submitHistory(60,options);
  },
