...
Controller.prototype = {
  ...
  onEnabled: function(options={}) {
    ...
    this._processingHistory = false;
    Services.obs.addObserver(this, kIdleDaily, false);
    return this.submitHistory(this._historyDaysToResubmit, options.flush)
  },
  ...
  submitHistory: function(daysAgo, doFlush) {
    ...
    this._processingHistoryPromise = Task.spawn(function() {
      ...
      this._currentReader = new HistoryReader(this._workers, this._dayBuffer, lastVisitId);
      yield this._currentReader.resubmitHistory({startDay: startDay});
      storage.lastVisitId = this._currentReader.getLastVisitId();
      if (doFlush) {
  ...
  resubmitHistory: function(options={}) {
    ...
    return this.submitHistory(this._historyDaysToResubmit, options.flush).then(() => {
