...
DayBuffer.prototype = {
  ...
  pushInterests: function(newVisitDate) {
    ...
    let lastDay = dates[0];
    let lastDayData = storage.dayBufferInterests[lastDay];
    delete storage.dayBufferInterests[lastDay];
    // remember what we want to push
    let dataToPush = storage.dayBufferInterests;
    // save lastDayData back to storage
    storage.dayBufferInterests = {};
    storage.dayBufferInterests[lastDay] = lastDayData;
    // and now we can push
