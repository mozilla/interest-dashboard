  ...
  consume: function(bucketData) {
    for (let day in bucketData) {
      ...
      if (typeObject && typeObject[this.namespace]) {
        for (let interest in typeObject[this.namespace]) {
          if (this.interests[interest] == null) {
            this.interests[interest] = 0;
          }
          this.interests[interest] += Object(typeObject[this.namespace][interest]).keys().length;
        }
      }
    }
    return bucketData;
