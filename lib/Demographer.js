...
Demographer.prototype = {
  ...
  getRules: function() {
    return this.rProc.getThresholdedResults();
  },
  ...
  readHistory: function(cb) {
    let query = "SELECT visit_count, rev_host, frecency, url, last_visit_date, title " +
                "FROM moz_places " +
                "WHERE rev_host IS NOT null AND visit_count > 0 AND hidden = 0";
