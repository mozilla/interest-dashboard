...
function Demographer(sitesCatFile, sitesDemogFile,sitesDemogGPFile) {
  this.ready = false;
...
Analyzer.prototype = {
  clearAll: function() {
    ...
    for (let x in demogBuckets) {
        this.demog[demogBuckets[x]] = {vtotal: 1, most: new ItemJar(15), least: new ItemJar(15)};
    }
    this.totalAcross = 0;
  },
  ...
  readHistory: function(cb) {
    ...
    if (this.daysBack) {
      query = " select COUNT(1), p.rev_host, frecency from moz_places p,moz_historyvisits v " +
  ...
  computeSiteWeight: function(domain) {
    let vcount = this.mySites[domain].count;
    if (vcount) {
    ...
    for (let index in orderedSites ) {
      ...
      if (siteData && siteData.demog) {
        ...
        for (let x in demogBuckets) {
          ...
          this.demog[buketName].most.addItem({domain: domain, vcount: bucketDrop}, bucketDrop)
          this.demog[buketName].least.addItem({domain: domain, vcount: bucketDrop}, -bucketDrop)
        }
      }
    }

    // normalize bukets into probabilites
    let total = this.demog["age_18"].vtotal + this.demog["age_25"].vtotal + this.demog["age_35"].vtotal +
                this.demog["age_45"].vtotal + this.demog["age_55"].vtotal + this.demog["age_65"].vtotal;

    this.demog["age_18"].vtotal /= total;
    this.demog["age_25"].vtotal /= total;
    this.demog["age_35"].vtotal /= total;
    this.demog["age_45"].vtotal /= total;
    this.demog["age_55"].vtotal /= total;
    this.demog["age_65"].vtotal /= total;

    total = this.demog["no_college"].vtotal + this.demog["some_college"].vtotal +
            this.demog["college"].vtotal + this.demog["graduate"].vtotal;

    this.demog["no_college"].vtotal /= total;
    this.demog["some_college"].vtotal /= total;
    this.demog["college"].vtotal /= total;
    this.demog["graduate"].vtotal /= total;

    total = this.demog["male"].vtotal + this.demog["female"].vtotal;

    this.demog["male"].vtotal /= total;
    this.demog["female"].vtotal /= total;

    total = this.demog["children"].vtotal + this.demog["no_children"].vtotal;

    this.demog["children"].vtotal /= total;
    this.demog["no_children"].vtotal /= total;
    // Object.keys(this.demog).forEach(function(key) { console.log(key,this.demog[key].vtotal); }.bind(this));
