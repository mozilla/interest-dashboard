...
const {mergeObjects, byteCount} = require("Utils");
const simplePrefs = require("simple-prefs")
	
const kIdleTimeoutSeconds = simplePrefs.prefs.dispatchIdleDelay || 5 * 60;
...
Dispatcher.prototype = {
  ...
  getPendingBatch: function _getPendingBatch() {
    if (Object.keys(storage.interests).length > 0) {
      return this._makePayload(kMaxPayloadSize);
    }
    return null;
  },
  ...
  _sendPing: function __sendPing(serverUrl) {
    ...
    return this._dispatch(serverUrl, payload).then(days => {
      ...
      Services.obs.notifyObservers(null, "dispatcher-payload-transmission-complete", days);
      return days.length;
    },
    reason => {
      return reason;
