...
HistoryReader.prototype = {
  ...
  handleEvent: function(aEvent) {
    ...
    if (eventType == "message") {
      ...
      if (msgData.messageId == "resubmit") {
        ...
        else if (msgData.message == "KeywordsForDocument") {
  ...
  _handleKeywordsResults: function I__handleKeywordsResults(aData) {
    if (aData.messageId == "resubmit") {
      this._keywordsWorkersData.push(aData);
      this._ResubmitRecentHistoryUrlCount.keywords--;
      if (this._ResubmitRecentHistoryUrlCount.keywords == 0) {
        let keywordData = [];
        this._keywordsWorkersData.forEach(data => {
            keywordData.push({details: data, dateVisits: this._urlCollector[data.url]["dates"]});
        });
        this._streamObjects.stream.push("keyword", keywordData);
        this._keywordsWorkersDeferred.resolve();
      }
    }
  },
  ...
  _resubmitRecentHistoryChunk: function I__ResubmitRecentHistoryChunk() {
    // clear out url count
    this._ResubmitRecentHistoryUrlCount = {interests: 0, keywords: 0};
    this._urlCollector = {};
    this._interestsWorkersData = [];
    this._keywordsWorkersData = [];
    this._tldCounter = {};
    this._chunkLastTimeStamp = this._ResubmitRecentHistoryLastTimeStamp;
    this._keywordsWorkersDeferred = Promise.defer();
    this._interestsWorkersDeferred = Promise.defer();

    Promise.all([this._keywordsWorkersDeferred.promise, this._interestsWorkersDeferred.promise]).then(() => {
      try {
    ...
    return PlacesInterestsUtils.getRecentHistory(this._ResubmitRecentHistoryStartDay, item => {
      try {
        ...
        this._urlCollector[url].message.visitIDs[item.visitDate].push(item.visitId);
    ...
    },
    {
      ...
      if (!this._stop) {
        Object.keys(this._urlCollector).forEach(url => {
          ...
          this._ResubmitRecentHistoryUrlCount.keywords += this._workers.keywords.length;
          ...
          this._workers.keywords.forEach(worker => {
            worker.postMessage({
              payload: this._urlCollector[url].message,
              command: "getKeywordsForDocument",
            });
          });
        });
      }
       ...
       if (this._ResubmitRecentHistoryDeferred && this._ResubmitRecentHistoryUrlCount.interests == 0 && this._ResubmitRecentHistoryUrlCount.keywords == 0) {
         this._resolveResubmitHistoryPromise();
       }
