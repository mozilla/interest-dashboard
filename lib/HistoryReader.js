...
HistoryReader.prototype = {
  ...
  setupWorker: function() {
    ...
    let dt = scriptLoader.loadSubScript(data.url("interestsData.js"));
    let model = scriptLoader.loadSubScript(data.url("interestsClassifierModel.js"));
    let stopwords = scriptLoader.loadSubScript(data.url("interestsUrlStopwords.js"));
    this._worker = new ChromeWorker(data.url("interestsWorker.js"));
  ...
  _handleInterestsResults: function I__handleInterestsResults(aData) {
    ...
    Task.spawn(function() {
      dump("Recieved categorization data:" + JSON.stringify(aData) + "\n");
