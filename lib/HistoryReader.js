      ...
      if (aData.messageId == "resubmit") {
        ...
        if (this._ResubmitRecentHistoryUrlCount == 0) {
          ...
          if (storage.tldCounter == null) {
            ...
            Object.keys(this._tldCounter).forEach(tld => {
              if (storage.tldCounter[tld] == null) {
                storage.tldCounter[tld] = 0;
              }
              storage.tldCounter[tld] += this._tldCounter[tld];
            });
          }
  ...
  _resubmitRecentHistoryChunk: function I__ResubmitRecentHistoryChunk() {
    ...
    return PlacesInterestsUtils.getRecentHistory(this._ResubmitRecentHistoryStartDay, item => {
      try {
        ...
        // update tldCounter
        let tld = this._urlCollector[url].publicSuffix;
        if (this._tldCounter[tld] == null) {
          this._tldCounter[tld] = 0;
        }
        this._tldCounter[tld]++;
      }
      catch(ex) {
