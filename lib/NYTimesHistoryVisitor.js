...
function NYTimesHistoryVisitor(storageBackend) {
  this.storage = storageBackend || storage;
}

NYTimesHistoryVisitor.prototype = {
  ...
  consumeHistoryVisit: function(visitData, visitMeta) {
    if (visitMeta.baseDomain != "nytimes.com") {
      return;
    }
    ...
    if (this.storage.nytimesVisits == null) {
      this.storage.nytimesVisits = [];
    }
    ...
    this.storage.nytimesVisits.push(visitObject);
  },

  getVisits: function() {
    return this.storage.nytimesVisits;
  },

  clear: function() {
    this.storage.nytimesVisits = null;
  },
};
