...
const SQL = {
  getRecentHistory:
    ...
    "JOIN (SELECT place_id, " +
          ...
          "WHERE visit_date >= (:startDay * :MS_PER_DAY*1000) AND " +
          "      visit_date < (:endDay * :MS_PER_DAY*1000) " +
...
let PlacesInterestsUtils = {
   ...
   *          endDay - only the records older then endday will be selected
   * @returns Promise for when all the recent pages have been processed
   */
  getRecentHistory: function PIS_getRecentHistory(startDay, handlePageForDay, options={}) {
    return this._execute(SQL.getRecentHistory, {
      ...
      params: {
        ...
        endDay: options.endDay || (DateUtils.today() + 1),
