...
TypeNamespace.deserialize = function(typeNamespaceJSON) {
  ...
  newTNObj.minDay = typeNamespaceJSON.minDay;
...
TypeNamespace.prototype = {
  ...
  _checkAndUpdateCapturedRankings: function() {
    ...
    if (!previousCapturedRankingExists && this.maxDay >= sevenDaysAgo) {
      ...
      this.capturedRankings.date = [today, todayTimestamp];
    }
    if (previousCapturedRankingExists && this.capturedRankings.date[0] <= sevenDaysAgo) {
  ...
  addDayToCategory: function(categoryName, day, domainsToCountMap, visitIDs, subcats) {
    if (day > this.maxDay) {
      this.maxDay = day;
    }
    if (day < this.minDay) {
    ...
    categoryObj.addDay(day, domainsToCountMap, visitIDs, subcats);
  },
