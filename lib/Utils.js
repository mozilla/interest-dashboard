...
exports.getHistoryDetails = function getHistoryDetails(category, historyIDs, visitListComplete, pageNum) {
  return PlacesInterestsUtils.getHistoryDetails(historyIDs, item => {
    ...
    let visitObj = {"timestamp": item.timestamp, "url": NetUtil.newURI(item.url).host, "title": item.title, "favicon": item.favicon};
    storage.chartData.interestDashboardData.historyVisits[category].visitData.push(visitObj);
  });
}
