...
exports.getHistoryDetails = function getHistoryDetails(category, historyIDs) {
  return PlacesInterestsUtils.getHistoryDetails(historyIDs, item => {
    ...
    if (!storage.chartData.interestDashboardData.historyVisits[category]) {
      storage.chartData.interestDashboardData.historyVisits[category] = [];
    }
    storage.chartData.interestDashboardData.historyVisits[category].push(
      {"timestamp": item.timestamp, "url": NetUtil.newURI(item.url).host, "title": item.title, "favicon": item.favicon});
  });
}

exports.getFaviconForHistoryVisit = function getFaviconForHistoryVisit(domainIndex, historyID) {
  return PlacesInterestsUtils.getFaviconForHistoryVisit(historyID, item => {
    storage.chartData.interestDashboardData.sortedDomains[domainIndex][2] = item;
  }, {"visitID": historyID});
}
