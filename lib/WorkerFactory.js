...
const INTEREST_LOCALES = {
  ...
  "en-US": {
    mainTaxonomyModel: "HTL",
    rankersDef: [
      {type: "rules", namespace: "edrules"}
    ],
    modelNames: ["58-cat", "edrules", "edrules_extended", "dfr_rules", "HTL", "HT", "HL"],
  },
};
...
WorkerFactory.prototype = {
  ...
  _readLocalizedInterests: function() {
    try {
      ...
      let localizedInterests = JSON.parse(data.load("models/" + this._localeCode + "/localizedInterests.json"));
  ...
  _setupInterestsWorker: function(modelName) {
    ...
    let interestsData = JSON.parse(data.load("models/" + this._localeCode + "/rules/" + modelName + ".json"));
