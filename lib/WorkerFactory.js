...
WorkerFactory.prototype = {
  ...
  _readLocalizedInterests: function() {
    try {
      ...
      let localizedInterests = JSON.parse(data.load("models/" + this._localeCode + "/" + mainTaxonomyModel + "/localizedInterests.json"));
  ...
  _setupInterestsWorker: function(modelName) {
    ...
    let interestsData = JSON.parse(data.load("models/" + this._localeCode + "/" + modelName + "/domainRules.json"));
