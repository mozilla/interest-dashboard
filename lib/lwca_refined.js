...
function LWCAClassifier(worker, callback) {
	...
	this.setDomainProgressCallback = function(callback) {
		cdb.setDomainProgressCallback(callback);
	}
...
function ComponentDatabase(worker, callback, create_objects = true) {
	...
	this.setDomainProgressCallback = function(callback) {
		this._domainProgressCallback = callback;
	}
	...
	this._handleAnalyzedDomain = function(msgData) {
		if (this._domainProgressCallback) {
			this._domainProgressCallback("domainProgress", msgData.domainCount, msgData.totalCount);
		}
	}
		...
		if (eventType == "message") {
			...
			switch (msgData.message) {
				...
				case "computedPTC":
					...
					break;
				case "domainAnalyzed":
					this._handleAnalyzedDomain(msgData);
					break;
			}
