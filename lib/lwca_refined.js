		...
		if (rule_mapping != false) {
			...
			if (words_tree.hasOwnProperty(rule_mapping)) {
				...
				if (vk_tree.hasOwnProperty(rule_mapping)) {
					top_level = vk_tree[rule_mapping]
					if(verbose) console.log('Used maxchunk to classify ' + url + " as " + [top_level, rule_mapping])
					console.log([top_level, rule_mapping])
					return [top_level, rule_mapping]
				}
			}
		}
...
function ComponentDatabase(worker, callback, create_objects = true) {
	...
	this.stop_everything = function(callback){
		this._worker.postMessage({
			command: "stop_everything",
			payload: {}
		});
	}
