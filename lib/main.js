...
// this requires are needed for WorkerProcessor loads
// starting from sdk-1.12, cfx xpi will not include
// lib/*js files that are not requires anywhere 
// hence we MUST require our Processor files here
...
exports.main = function(options, callbacks) {
  ...
  PageMod({
    contentScriptFile: [
      data.url("dashboard/jquery/jquery.min.js"),
    ...
    onAttach: function(worker) {
      worker.port.on("donedoc", function() {
        ...
        demographer.getCategories(function(cats) {
          worker.port.emit("show_cats", cats);
        });

        demographer.getDemographics(function(demogs) {
          worker.port.emit("show_demog", demogs);
        });

        demographer.getRules(function(rules) {
  ...
  PageMod({
    ...
    onAttach: function(worker) {
      if(worker.url.indexOf("http") == 0) {
        demographer.consumeOnePageVisit(worker.url, worker.tab.title);
      }
    }
  });
