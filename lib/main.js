  ...
  PageMod({
    ...
    onAttach: function(worker) {
      worker.port.on("history_run", function() {
        gController.submitHistory().then(() => {
          worker.port.emit("history_done");
        });
      });

      worker.port.on("history_data", function() {
        worker.port.emit("history_data", gController.getHistoryData());
      });
    }
  });
