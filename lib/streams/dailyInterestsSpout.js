...
let DailyInterestsSpout = {
  create: function _DIS_create(storageBackend) {
    let node = createNode({
      ...
      _storeInterest: function _DIS__storeInterest(host, visitIDs, visitDate, visitCount, namespace, type, interest) {
        ...
        if (!this.storage.dayBufferInterests[visitDate][type][namespace][interest]) {
          this.storage.dayBufferInterests[visitDate][type][namespace][interest] = {"hosts": {}, "visitIDs": []};
        }
        ...
        this.storage.dayBufferInterests[visitDate][type][namespace][interest].visitIDs =
          this.storage.dayBufferInterests[visitDate][type][namespace][interest].visitIDs.concat(visitIDs[0]);
      },

      _addDomain: function(host, visitCount, visitID) {
        if (!host) {
        ...
        if (!storage["domains"][host]) {
          storage["domains"][host] = {"count": 0, "visitID": -1};
        }
        if (storage["domains"][host].visitID == -1) {
          storage["domains"][host].visitID = visitID;
        }
        storage["domains"][host].count += visitCount;
      },
      ...
      ingest: function _DIS_ingest(message) {
        for(let i=0; i < message.length; i++) {
          ...
          Object.keys(dateVisits).forEach(date => {
            this._addDomain(host, dateVisits[date], visitIDs[0]);
          });
          ...
          results.forEach(item => {
            ...
            interests.forEach(interest => {
              Object.keys(dateVisits).forEach(date => {
                this._storeInterest(host, visitIDs, date, dateVisits[date], namespace, type, interest);
              });
            });
          });
        }
