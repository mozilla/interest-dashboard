...
let DailyInterestsSpout = {
  create: function _DIS_create(storageBackend) {
    let node = createNode({
      ...
      _storeInterest: function _DIS__storeInterest(host, visitID, visitDate, visitCount, namespace, type, interest) {
        ...
        this.storage.dayBufferInterests[visitDate][type][namespace][interest].visitIDs.push(visitID);
      },

      _addDomain: function(host, visitCount) {
        if (!storage["domains"]) {
          storage["domains"] = {};
        }
        if (!storage["domains"][host]) {
          storage["domains"][host] = 0;
        }
        storage["domains"][host] += visitCount;
      },

      ingest: function _DIS_ingest(message) {
        for(let i=0; i < message.length; i++) {
          let {details, dateVisits} = message[i];
          let {host, visitID, visitDate, visitCount, namespace, results} = details;
          Object.keys(dateVisits).forEach(date => {
            this._addDomain(host, dateVisits[date]);
          });
          ...
          results.forEach(item => {
            ...
            interests.forEach(interest => {
              Object.keys(dateVisits).forEach(date => {
                this._storeInterest(host, visitID, date, dateVisits[date], namespace, type, interest);
              });
            });
          });
        }
