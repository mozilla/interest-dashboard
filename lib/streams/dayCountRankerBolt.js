...
let DayCountRankerBolt = {
  batchCreate: function _DCRB_batchCreate(batch, storageBackend) {
    let rankers = [];
    batch.forEach(def => {
      rankers.push(DayCountRankerBolt.create(def.namespace, def.type, storageBackend));
    });
    return rankers;
  },
  ...
  create: function _DCRB_create(namespace, type, storageBackend) {
    ...
    let node = createNode({
      ...
      getInterests: function _DIR_getInterests() {
        if (this.storage.ranking && this.storageKey && this.storage.ranking[this.storageKey] && Object.keys(this.storage.ranking[this.storageKey]).length > 0) {
      ...
      clearData: function _DIR_clear() {
        this.interests = {};
        if (this.storage.ranking && this.storage.ranking[this.storageKey]) {
          this.storage.ranking[this.storageKey] = {};
        }
      },

      clearStorage: function _DIR_clearStorage() {
        delete this.storage.ranking[this.storageKey];
      }
