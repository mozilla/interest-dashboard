  ...
  create: function _CDPB_create(storageBackend) {
    let node = createNode({
      ...
      _getSortedDomains: function(domainsObjs) {
        ...
        return {"all": allSorted, "byInterest": byInterestSorted}
      },
      ...
      getFaviconsForTopSites: function() {
        let topDomains = storage.chartData.interestDashboardData.sortedDomains.all;
        let topDomainsForInterests = storage.chartData.interestDashboardData.sortedDomains.byInterest;
        return Task.spawn(function() {
          for (let domain of topDomains) {
            let visitID = domain[2];
            yield getFaviconForHistoryVisit(domain, visitID);
          }
          for (let interest in topDomainsForInterests) {
            for (let domain of topDomainsForInterests[interest]) {
              let visitID = domain[2];
              yield getFaviconForHistoryVisit(domain, visitID);
            }
          }
      ...
      ingest: function _HSB_ingest(message) {
        try {
          ...
          this.storage.chartData.interestDashboardData.sortedDomains =
            this._getSortedDomains(this.storage.domains);
