    ...
    let node = createNode({
      ...
      getNextHistoryVisitPage: function(categoryName) {
        return Task.spawn(function() {
          ...
          yield getHistoryDetails(categoryName, visitQueryList, startIndex == 0, pageNum, category.subcats);
        });
      },
      ...
      getFaviconsForTopSites: function(category) {
        try {
          ...
          if (!category) {
            ...
            let allSortedDomains = this._sortDomains(domainsObjs.all);
            for (let domain of allSortedDomains) {
              if (sortedDomains.all.length == 10) {
                break;
              }
              if (!shouldSkip(domain[0])) {
                sortedDomains.all.push(domain);
              }
            }
            return Task.spawn(function() {
